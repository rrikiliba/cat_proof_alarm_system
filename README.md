...
- Since all the functionality of the controller device is achieved through Docker containers, you don't actually need an Orange Pi Zero 2W to run that portion of the project. You can run it on any device (embedded or not) and, by connecting it to an I²C port on your device and configuring the correct parameters in the .env file, you can also optionally use the external OLED display as instructed above.
